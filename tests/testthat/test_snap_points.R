test_that("snap_points", {
  ...
  res <- 15
  result <- cpp_snap_points_nearest(stops %>% sf::st_coordinates(), stops %>% sf::st_coordinates(), res)
  
  expect_equal(result, 1)

  expect_length(cpp_snap_points_nearest(stops %>% sf::st_coordinates(), matrix(2, 1), res), 0)
  ...
  res <- 3000
  result <- cpp_snap_points_restrictive(stops %>% sf::st_coordinates(), shape_points %>% sf::st_coordinates(), res)
