...
# dt <- subset(poa_gps, trip_id %in% c('176-1@1#1028' , 'A141-1@1#1750', 'R10-2@1#1010'))
dt <- poa_gps[ between(departure_time, lower = as.ITime("07:00:00"), upper = as.ITime("12:30:00")) ]
...
ggplot() +
    geom_path(data=dt, aes(color = trip_id,  x=shape_pt_lon, y=shape_pt_lat), show.legend = FALSE, alpha = 0.2 ) +
    # geom_point(show.legend = FALSE, alpha = 0.7) +
    scale_color_viridis_d() +
    # Here comes the gganimate specific bits
