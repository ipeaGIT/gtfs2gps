...
These functions subset all the relevant GTFS files in order to remove all the unnecessary rows, keeping the data consistent. The returning values of the three functions is a `data.table`. For example, in the code below we filter only shape ids between 53000 and 53020.
...
To convert GTFS to GPS format, use `gtfs2gps_dt_single()`. This
...
```{r}
  sp_gps <- gtfs2gps_dt_single("sp_small.zip", progress = FALSE)
  sp_gps
```
...
Function `gtfs2gps_dt_single()` also works with GTFS data that do not have frequency.txt. It automatically recognises the format and then it uses only stop_times.txt to create the GPS data.
