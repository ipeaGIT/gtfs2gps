...
date: "`r format('15 Decembrt 2019')`"
output:
  pdf_document:
    fig_caption: yes
...
Package `gtfs2gps` allows users to convert public transport GTFS data into a single `data.table` format with GPS-like records. Before using the package, just install it from GitHub.
...
sp_small <- gtfs2gps::filter_by_shape_id(sp, c(51338, 51956, 51657))
...
stops_small_sf <- gtfs2gps::gtfs_stops_as_sf(sp_small)

plot(sf::st_geometry(sp_small_sf))
plot(sf::st_geometry(stops_sf), pch = 20, col = "red", add = TRUE)
box()
```
...
To convert GTFS to GPS-like format, use the `gtfs2gps()`. This
function takes a GTFS zipped file as argument and returns a
`data.table` where each row works as a GPS sample for a given
trip in the GTFS file. In summary, this function interpolates
the space-time position of each vehicle for each trip given 
its route path and average speed between stops. See the example
as follows.

```{r}
  sp_gps <- gtfs2gps("sp_small.zip", progress = FALSE, cores = 1)
  sp_gps
```

For a given trip, the function `gtfs2gps` calculates  different ranges of average speed between stops --- given by the ratio between cumulative distance $S$ and departure time $t$ for a consecutive pair of valid stop_ids ($i$), 
$$
...
Function `gtfs2gps()` also works with GTFS data that do not have frequency.txt. It automatically recognises the format and then it uses only stop_times.txt to create the GPS-like data.
