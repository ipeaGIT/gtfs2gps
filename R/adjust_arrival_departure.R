...
#' time on stops. For each stop time row, this function applies the following steps:
#' 
#' 1. If there is `arrival_time` but no `departure_time`, it creates a departure 
#' by summing the arrival plus a pre-defined `min_lag`.
#' 
#' 2. If there is `departure_time` but no `arrival_time`, it creates an arrival 
#' by subtracting a pre-defined `min_lag` from the departure.
#' 
#' 3. If there is an `arrival_time` and a `departure_time` but their difference
#' is less than `min_lag`, it reduces the `arrival_time` and increases
...
#' @param min_lag Minimum waiting time when a vehicle arrives at a stop, in
#' seconds (units). Default is 20s.
...
adjust_arrival_departure <- function(gtfs_data, min_lag = 20){
  min_lag <- units::set_units(min_lag, "s") %>%
    as.numeric()
