...
#' It will load the following files: "calendar.txt", "routes.txt", "shapes.txt", 
...
read_gtfs <- function(gtfszip){
  ...
  if("stop_times.txt"  %in% unzippedfiles){result$stoptimes   <- data.table::fread(paste0(tempd,"/stop_times.txt"),  encoding="UTF-8")}  else{stop(message("Error: File stop_times.txt is missing"))}
